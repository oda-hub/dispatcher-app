...
class SpectralFitProduct(BaseQueryProduct):
    ...
    def run_fit(self, e_min_kev, e_max_kev, plot=False, xspec_model='powerlaw', params_setting=None, frozen_list=None):
        ...
        if msk.sum() > 0:
            ...
            # ax1.set_ylim(-3,1)
            sp1 = ScatterPlot(w=600, h=400, x_label='log (Energy) (keV)', y_label='log (normalize counts/s/keV)',y_range=[-3,1])
            #print('OK 1a')
            ...
            sp2 = ScatterPlot(w=600, h=40, x_label='log (Energy) (keV)', y_label='(data-model)/error',
